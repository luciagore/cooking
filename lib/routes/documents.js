...
router.get('/documentlist', function(req, res) {
    ...
    apiActions.getDocuments(req.db.get(config.collectionName), userId, function(data) {
        res.json(data);
    });
});
...
router.get('/documentlist/:query', function(req, res) {
    ...
    apiActions.getDocumentByContent(req.db.get(config.collectionName), userId, query, function (data) {
        res.json(data);
    });
});

router.post('/adddocument', function(req, res) {
    var collection = req.db.get(config.collectionName);
