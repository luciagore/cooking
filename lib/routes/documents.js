...
router.get('/documentlist', function(req, res) {
    ...
    apiActions.getRecipes(req.db.get(config.collectionName), userId, function(data) {
        res.json(data);
    });
});
...
router.get('/documentlist/:query', function(req, res) {
    ...
    apiActions.getRecipeByIngredients(req.db.get(config.collectionName), userId, query, function (data) {
        res.json(data);
    });
});
